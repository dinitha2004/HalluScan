...
write("7_backend_colab.ipynb", [
    ...
    md("### 4 · Wait for model load + smoke-test\n"
       "Waits for the API to come up (the server only answers **after** the model loads — the first run downloads "
       "~16 GB, so this can take ~8–10 min). It only runs the test questions once `/status` reports the model is "
       "loaded; if it's still loading after the wait, it prints a clear message to re-run this cell — **no error.** "
       "Check `regime=sentence`, `sentence_tag=s1`, and the calibrated `t_med`/`t_high` in the status. After it "
       "succeeds, open the Vercel site (no pasting needed) and ask away."),
    code("import requests, time\n"
         ...
         "        try:\n"
         "            out = requests.post('http://localhost:8000/infer', json={'question': q}, timeout=180).json()\n"
         "            agg = out['aggregate']\n"
         "            print(f\"\\nQ: {q}\\nA: {out['answer'][:160]}\")\n"
